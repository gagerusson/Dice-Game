...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal: 
        if next_player(who) == 1: #player 0 is curr
            rolls = strategy0(score0, score1)
            score0 += take_turn(rolls, score1, dice, goal)
            score0 += hog_pile(score0, score1)
            who += 1
        else:
            rolls = strategy1(score1, score0)
            score1 += take_turn(rolls, score0, dice, goal)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def announce(score0, score1):
        ...
        if score_change > running_high:
            print(f"{str(score_change)} point(s)! That's a record gain for Player {str(who)}!")
            
        highest = max(score_change, running_high)
        return announce_highest(who, curr_score, highest)
    return announce
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    optimal = 0
    for i in range(1, 11):
        average_value = make_averaged(roll_dice, trials_count)(i, dice)
        if average_value > optimal:
            num_dice = i
            optimal = average_value     
    return num_dice
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return 0 if ((picky_piggy_strategy(score, opponent_score, cutoff, num_rolls) == 0) or (opponent_score - picky_piggy(opponent_score)) == score) else num_rolls
    # END PROBLEM 11
